...
const searchCoinEvents = (model, populate) => async (req, res, next) => {
  ...
  //Fitering query parms
  Object.keys(JSON.parse(queryStr)).length > 0 &&
    (await Promise.all(
      Object.keys(JSON.parse(queryStr)).map((i) => {
        filterArray.push({ [i]: JSON.parse(queryStr)[i] });
      })
    ));
  ...
  query =
    !Object.keys(JSON.parse(queryStr)).length > 0
      ? model.find({ dateEvent: { $gt: new Date(Date.now()) } })
      : model.find({
          $and: [
            { $or: filterArray },
