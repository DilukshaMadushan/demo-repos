...
const CoinsDropdown = ({
  ...
  getCryptoNewsCoins,
  getCryptoNewsFilteredCoins,
  cryptoNews: { loadingCryptoNewsCoins, cryptoNewsCoinsList },
}) => {
  const DUMMY_LIST = [
    { id: "1" },
    { id: "2" },
  ...
  const filterCoinList = (coinName) => {
    ...
    if (coinName === "") {
      getCryptoNewsFilteredCoins(filterURL);
    }
            ...
            <div className={`${styles.dropdownContent}`}>
              ...
              {loadingCryptoNewsCoins
                ...
                : cryptoNewsCoinsList !== null &&
                  cryptoNewsCoinsList.map((item, index) => (
                    <div
                      onClick={() => {
                        setCoinsDropdownSelected(item.symbol);
                        setIsCoinsDropdownActive(false);
                        handleChange(item._id);
                        handleDateChangeforSentimentChart(item._id);
                      }}
                      className={`${styles.dropdownItemDiv}`}
                      key={index}
                    >
...
CoinsDropdown.propTypes = {
  getCryptoNewsCoins: PropTypes.func.isRequired,
  getCryptoNewsFilteredCoins: PropTypes.func.isRequired,
};
